;(function(){
  // base URL de votre GitHub Pages
  ...
  function init(){
    ...
    // Injecter juste avant le script qui appelle ce fichier
    ...
    const rules = [
      ...
      {desc:"Cookies marketing", rx:/(?:_ga\\d?|_gid|_gat|_fbp|gclaw|gcldc|li_fat_id|hubspotutk|calltrk|nimbata|mkto_trk|optimizely|ajs\\w*)=/i, pts:-20},
      {desc:"Tracking en dur", rx:/(?:connect\\.facebook\\.net|analytics\\.tiktok\\.com|px\\.ads\\.linkedin\\.com|cdn\\.segment\\.com|hotjar|clarity|callrail)/i, pts:-10},
      {desc:"Server-side tracking", rx:/https:\\/\\/(analytics|sst|tracking)\\./i, pts:-10, invert:true},
    ...
    btn.addEventListener('click', async ()=>{
      ...
      try{ new URL(url) }catch{ showError('URL invalide'); return }
      toggle(true); hideError(); results.style.display='none'
      ...
      try{
        ...
        let score=100, found=[]
        rules.forEach(r=>{
          const m=r.rx.test(html)
          if((r.invert?!m:m)){ score+=r.pts; found.push(r) }
        })
        const dup=(html.match(/<script[^>]+(gtm\.js|fbevents\.js|analytics\.js)/gi)||[]).length
        if(dup>1){ score-=5; found.push({desc:"Double balisage",pts:-5}) }
        ...
        issuesEl.innerHTML = found.sort((a,b)=>a.pts-b.pts).slice(0,3)
          .map(r=>`<li class="tc-issue">${r.desc} (${r.pts>0?'+':''}${r.pts} pts)</li>`).join('') ||
    ...
    function showError(txt){
      errorEl.textContent=txt; errorEl.style.display='block'
    }
    function hideError(){
